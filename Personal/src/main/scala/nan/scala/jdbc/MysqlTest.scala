...
object MysqlTest {
  ...
  ConnectionPool.singleton("jdbc:mysql://192.168.12.41:3306/ezp-bd", "ezwrite", "33KlsXareQbsbrfhq2eJ")

  implicit val session = AutoSession

  def mysqlSwitchJavaType(from: String): String = {
    if (from.contains("int")) "Long"
    else if (from.contains("char") || from.contains("datetime") ||from.contains("text")) "String"
  ...
  def getCustomField(b: Int):ArrayBuffer[FieldSchema] = {
    ...
    i += 1
    val consumeTimestamp = FieldSchema("ConsumeTimestamp", "Long", i, "", 0, "", 0)
    i += 1
    val canalTimestamp = FieldSchema("CanalTimestamp", "Long", i, "", 0, "", 0)
    i += 1
    val originTableName = FieldSchema("OriginTableName", "String", i, "", 0, "", 0)
    ArrayBuffer(dbSharId, cloudType, opType, pushTimestamp, consumeTimestamp, canalTimestamp, originTableName)
  }

  def main(args: Array[String]): Unit = {

    val entities: List[Map[String, Any]] = sql"select * from mp_msg_grpmsg".map(_.toMap).list.apply()

    val desc: List[Map[String, Any]] = sql"desc mp_msg_grpmsg".map(_.toMap).list.apply()
